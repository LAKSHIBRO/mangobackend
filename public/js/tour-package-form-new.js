...
/**
 * Initialize the main TinyMCE instances
 */
...
function initIncludedExcludedItems() {
    ...
    if (addIncludedItemBtn) {
        addIncludedItemBtn.addEventListener('click', function() {
            addItem('includedItemsContainer', 'included[]', 'Enter included item');
        });
    }
    ...
    if (addExcludedItemBtn) {
        addExcludedItemBtn.addEventListener('click', function() {
            addItem('excludedItemsContainer', 'excluded[]', 'Enter excluded item');
        });
    }
...
function initItineraryFunctionality() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.remove-itinerary')) {
            ...
            if (container && itineraryItem) {
                // Remove the itinerary day
...
function addItineraryDay() {
    ...
    newDay.innerHTML = `
        <div class="card-header bg-light d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Day ${window.dayCounter}</h5>
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-12 mb-3">
                    ...
                    <textarea class="form-control itinerary-description" name="itinerary[${window.dayCounter-1}][description]" rows="4" required></textarea>
                </div>
    ...
    setTimeout(function() {
        if (typeof tinymce !== 'undefined') {
            tinymce.init({
                selector: `textarea[name="itinerary[${window.dayCounter-1}][description]"]`,
                plugins: 'anchor autolink charmap codesample emoticons image link lists media searchreplace table visualblocks wordcount',
                toolbar: 'undo redo | blocks fontfamily fontsize | bold italic underline strikethrough | link image media table | align lineheight | numlist bullist indent outdent | emoticons charmap | removeformat',
                height: 200
            });
