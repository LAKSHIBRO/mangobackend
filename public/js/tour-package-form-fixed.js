        ...
        <div class="card-header d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, #02515A 0%, #086571 100%); border-bottom: 0;">
            <div class="d-flex align-items-center">
                <div class="day-badge me-3 d-flex align-items-center justify-content-center" style="width: 45px; height: 45px; background: rgba(255,255,255,0.2); border-radius: 50%;">
                    <span class="text-white fw-bold">${dayCounter}</span>
                </div>
                <div>
                    <h5 class="mb-0 text-white fw-bold"><i class="fi fi-rr-calendar-day me-2"></i> Day ${dayCounter}</h5>
                    <small class="text-white-50">Itinerary Details</small>
                </div>
            </div>
            <button type="button" class="btn btn-sm btn-danger remove-itinerary" style="background-color: rgba(220, 53, 69, 0.8); border: none; backdrop-filter: blur(5px);">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control" name="itinerary[${dayCounter-1}][title]" placeholder="E.g. Arrival and Welcome" required>
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label">Location <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" name="itinerary[${dayCounter-1}][location]" placeholder="E.g. Kathmandu" required>
                </div>

                <div class="col-12 mb-3">
                    <label class="form-label">Description <span class="text-danger">*</span></label>
                    <textarea id="${textareaId}" class="form-control itinerary-description" name="itinerary[${dayCounter-1}][description]" rows="4" required></textarea>
                </div>
...
function removeItineraryDay(button) {
    ...
    setTimeout(() => {
        ...
        days.forEach((day, index) => {
            ...
            // Update heading text and day badge
            const dayHeading = day.querySelector('h5');
            if (dayHeading) {
                dayHeading.innerHTML = `<i class="fi fi-rr-calendar-day me-2"></i> Day ${dayNumber}`;
            }

            const dayBadge = day.querySelector('.day-badge span');
            if (dayBadge) {
